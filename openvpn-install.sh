...
function new_client() {    
    client=$2
    initial_client_name=$initial_client_name_override || "client"

    [[ -z "$client" ]] && client=$initial_client_name

    if [[ -z "$client" ]] && [[ -z $headless_mode ]]; then
    ...
    if ! pushd /etc/openvpn/server/easy-rsa/; then
...
ExecStart=$ip6tables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT" >> /etc/init.d/openvpn-iptables
    fi
    echo "
    ;;
  stop)
...
function uninstall_openvpn(){
    ...
    if [[ "$remove" =~ ^[yY]$ ]]; then
        # TODO change from systemctl to sysvinit
